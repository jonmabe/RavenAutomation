    ...
    async def main_loop(self):
        ...
        try:
            ...
            self.tasks = [
                ...
                asyncio.create_task(self.process_microphone())  # Add microphone processing task
            ]
                ...
                if not self.is_speaking:
                    if current_time - self.last_idle_time > self.idle_interval + np.random.uniform(0, self.idle_variance):
                        if self.head_movement_type == 'side':
                            ...
                            self.head_rotation_next_position = 0.7 if self.head_looking_left else 0.3
                            self.head_movement_type = 'tilt' if np.random.random() < 0.3 else 'side'
                        else:
                            self.head_tilt_next_position = np.random.uniform(0.3, 0.7)
                            self.head_movement_type = 'side'
                        
                        self.last_idle_time = current_time
                        self.idle_interval = 1.0 + np.random.uniform(-0.3, 0.3)
    ...
    async def process_microphone(self):
        ...
        try:
            while self.running and self.recording_stream:
                if not self.is_speaking:  # Only process mic when not playing audio
                    try:
                        data = self.recording_stream.read(self.CHUNK, exception_on_overflow=False)
                        if self.ws:  # Just check if we have a connection
                            try:
                ...
                await asyncio.sleep(0.0001)  # Reduced delay to 0.1ms
